...
How the game master starts a session (also a new session after game-over; [program.md](program.md) **D4**). Connect, sign, and send follow **F0**. HP formula: [arbiter.md](arbiter.md) **A3**. Isolation: [arbiter.md](arbiter.md) **A6**, [deployment.md](deployment.md) **DEP3**. Mint and vault authority: [deployment.md](deployment.md) **DEP5**, **DEP6**. PDAs: [deployment.md](deployment.md) **DEP1**.
...
| --- | --- | --- |
| `ConfidentialMint` (CPI during Initialize) | Encrypted HP into this instance vault’s **pending** balance on the shared HP mint. | HP mint authority (arbiter backend partial-sign) |
| `ApplyPendingBalance` (next instruction) | Folds pending into **available**. Required before any Attack burn. | HP vault authority (arbiter backend partial-sign; not a PDA) |
...
sequenceDiagram
  ...
  Note over BE: Price reward (A4). Draw offset. Set HP (A3).<br/>Vault ElGamal, supply ElGamal/AES, mint authority, and vault authority stay on the backend (A2, DEP2, DEP5, DEP6).
  BE->>BE: generate ElGamal ZK proofs
  BE->>BE: build Initialize plus ApplyPendingBalance
  BE->>BE: partial-sign as HP mint authority (ConfidentialMint) and vault authority (ApplyPendingBalance)
  ...
  Note over Prog: Initialize CPI ConfidentialMint (pending).<br/>ApplyPendingBalance immediately after (available; arbiter vault authority).<br/>Session live. Public reward locked. Shared HP mint is not created per instance.
```

**Figure 1.** Initialize sequence. GM connects with a wallet adapter, requests Init, completes the arbiter’s partially signed transaction as fee payer, and sends via the wallet RPC. The Piñata program initializes and confidential-mints HP into this instance’s vault on the shared mint; `ApplyPendingBalance` follows immediately so that HP is available (arbiter vault authority).
...
4. **Build.** Backend MUST assemble Initialize (CPI `ConfidentialMint`) and MUST put `ApplyPendingBalance` immediately after as the next instruction. The arbiter MUST partial-sign that apply as HP vault authority (**DEP6**). v1 MUST NOT PDA-sign it. Backend MUST partial-sign as HP mint authority (`ConfidentialMint`, **DEP5**) and MUST supply the new decryptable available balance for `ApplyPendingBalance` (vault AES). Frontend MUST return that transaction to the GM wallet for completion (**F0**).
...
6. **On chain.** Piñata program MUST initialize the instance. Solana runtime MUST allocate that instance’s PDAs (including the HP vault token account). Initialize MUST confidential-mint HP into that vault’s pending on the shared mint. `ApplyPendingBalance` MUST follow immediately, authorized by the arbiter as vault authority, so minted HP is available. Session is live; public reward is locked.

## F2. Attack

How a registered player strikes. Connect, sign, and send follow **F0**. On-chain result: [program.md](program.md) **D3**. Proofs and isolation: [arbiter.md](arbiter.md) **A2**, **A5**. Mint and vault authority: [deployment.md](deployment.md) **DEP5**, **DEP6**.
...
| Instruction | What it does | Who authorizes |
| --- | --- | --- |
| `ConfidentialBurn` | Homomorphic −1 on this instance’s HP vault. This **is** the hit. CPI from Attack. | HP vault authority (arbiter backend partial-sign) plus burn proofs |
| `ApplyPendingBurn` | Folds the shared mint’s `pending_burn` into encrypted supply. **Not** the HP decrement. | HP mint authority (arbiter backend partial-sign) |
...
Omitting the kill proof when leftover HP is actually 0 is an on-chain instruction gap (**A7**). Only a malicious or exploited arbiter can land that Attack; a player cannot, because Attack is already arbiter-partial-signed (**F0**). The arbiter is assumed trustworthy and MUST NOT assemble it (**A5**).
...
sequenceDiagram
  ...
  Note over BE: Vault ElGamal, supply ElGamal/AES, mint authority, and vault authority stay on the backend (A2, DEP2, DEP5, DEP6).<br/>MUST NOT reveal HP, offset, or range (A5).
  ...
  BE->>BE: partial-sign ConfidentialBurn as vault authority; ApplyPendingBurn then UpdateDecryptableSupply as mint authority
  BE-->>FE: transaction(s)
  ...
  alt no VerifyZeroCiphertext in Attack
    Note over Prog: Stay live (D3). Only a malicious/exploited arbiter omits (A7).<br/>Conforming arbiter MUST NOT (A5).
...
4. **Build.** Backend MUST assemble: proof setup as needed → Attack with `ConfidentialBurn` CPI (HP − 1; arbiter vault-authority signature, **DEP6**) and, on a kill, `VerifyZeroCiphertext` → `ApplyPendingBurn` immediately after → `UpdateDecryptableSupply`. Arbiter MUST partial-sign `ApplyPendingBurn` and `UpdateDecryptableSupply` as HP mint authority (**DEP5**).
...
3. Where a Token-2022 instruction requires HP mint authority, that arbiter partial-sign is **DEP5**. Where it requires HP vault authority (`ConfidentialBurn`, `ApplyPendingBalance`, vault close), that partial-sign is **DEP6**. Attack also requires burn proofs only the backend can produce; a player MUST NOT be able to assemble a competing Attack from an unsigned payload.
...
Close is arbiter-constructed (**DEP4**, [program.md](program.md) **D4**). The shared HP mint MUST NOT be closed with the instance (**DEP6**). Unspecified: the exact Token-2022 close and leftover-zero proof instructions besides `VerifyZeroCiphertext` bound to the HP vault; Register/Close sequence diagrams.
